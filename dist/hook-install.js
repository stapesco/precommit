 ...
 */
function writeHook(path, content) {
...
export function installHook(repoRoot) {
    ...
    if (existsSync(hookPath)) {
        ...
        process.stdout.write(`Existing hook preserved. stapes-precommit appended to ${hookPath}\n`);
        return 0;
    }
...
export function uninstallHook(repoRoot) {
    ...
    const existing = readFileSync(hookPath, "utf8");
    if (!existing.includes(HOOK_MARKER)) {
        process.stderr.write(`${c.red("error:")} existing hook is not from stapes-precommit. Skipping.\n`);
        return 1;
    }
    // Strip our block.
