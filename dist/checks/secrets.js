 ...
 */
import { DEFAULT_PATTERNS, SECRET_SCAN_EXCLUDE } from "../patterns.js";
export const defaultSecretsConfig = {
    enabled: true,
    extra_patterns: [],
};
export function checkSecrets(files, config) {
    const findings = [];
    const patterns = [...DEFAULT_PATTERNS, ...config.extra_patterns];
    for (const file of files) {
        if (!file.content)
            continue;
        if (SECRET_SCAN_EXCLUDE.some((rx) => rx.test(file.path)))
            continue;
